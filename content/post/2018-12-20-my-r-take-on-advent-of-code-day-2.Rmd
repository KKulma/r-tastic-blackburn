...
Right, Advent of Code will never give you nice and clean data to work with, that's for sure. But it doesn't looks like things are too bad this time - let's just split it by the new line and keep it as a vector for now. Does it look reaosnably well?
...
Now, the way I approached this was to split each word into letters and then count how many times they occured. Then, for identifying words with 2 occurences, I'd filter I filtered only those that occur twice and if the final table has any rows, then this counts as yes. Take the first example:
...
Definitely yes. Let's repeat the process for tribble occurances:
...
Not much luck with those. To make our life easier, let's wrap both calculatioons in functions...
...
Now all we need to do it check how many positive elements we have in each vector and multiple their lengths by each other:
